...
# Ẩn đoạn text lỗi của icon sidebar
st.markdown("""
    <style>
    button[kind="headerNoContextualWidget"] div {
...
if df is not None:
    ...
    with st.sidebar:
        ...
        if os.path.exists("https://i.ibb.co/dwkNgdxm/Thumb-Luoc-su.jpg"):
            st.image("https://i.ibb.co/dwkNgdxm/Thumb-Luoc-su.jpg", use_container_width=True)
        else:
            st.markdown("<div style='background-color:#FF9A9E; padding:50px; border-radius:20px; text-align:center; color:white;'>📸<br>Chỗ này để ảnh xe cực cháy</div>", unsafe_allow_html=True)
        
        st.markdown("---")
        st.write(f"🌈 **Tình trạng:** Đang cực 'mượt'")
        count_xe = df["Hãng xe"].count()
        st.write(f"📊 **Data:** {count_xe} 'em' xe")
