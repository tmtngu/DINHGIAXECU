...
st.set_page_config(page_title="TECHBEES | Slay & Ride", page_icon="🛵", layout="wide")

# CSS Custom: Style này là sự kết hợp giữa sự chuyên nghiệp và chút "dễ thương"
st.markdown("""
    <style>
    /* --- PHẦN 1: DIỆT CHỮ LỖI (FIX TÙM LUM) --- */
    /* Ẩn chữ lỗi ở nút đóng/mở Sidebar */
    [data-testid="stSidebarCollapse"] div div {
        display: none !important;
    }
    
    /* Ẩn chữ lỗi ở Menu cài đặt góc phải (Light/Dark mode) */
    .st-emotion-cache-17686p6, [data-testid="stStatusWidget"], .st-emotion-cache-10o0f9z {
        font-size: 0px !important;
    }

    /* Ẩn các icon bị lỗi font chữ hệ thống */
    span[data-testid="stIconMaterial"] {
        display: none !important;
    }

    /* --- PHẦN 2: GIỮ STYLE GEN Z CỦA M --- */
        ...
        with right_col:
            ...
            if predict_btn:
                with st.spinner("Đợi tí, AI đang 'cook' dữ liệu..."):
                    ...
                    st.write("")
                    st.metric(f"{model_bike}", f"{res:,.0f}đ", f"{diff:,.0f}đ", delta_color=delta_color)
            else:
